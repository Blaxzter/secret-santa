...
function getOrCreateUserId(): string {
  const stored = localStorage.getItem("secret_santa_user_id");
  if (stored) return stored;

  const newId =
    Math.random().toString(36).substring(2, 15) +
    Math.random().toString(36).substring(2, 15);
  localStorage.setItem("secret_santa_user_id", newId);
  return newId;
}

export function getUserId(): string {
  return localStorage.getItem("secret_santa_user_id") || "";
}

export function setUserId(id: string): void {
  localStorage.setItem("secret_santa_user_id", id);
}
...
const translations = {
  de: {
    ...
    bulkImport: "Namen einfügen",
    bulkImportPlaceholder:
      "Namen kommagetrennt eingeben (z.B. Anna, Ben, Clara)",
    importNames: "Namen importieren",
  },
  en: {
    title: "Secret Santa",
    ...
    bulkImport: "Paste Names",
    bulkImportPlaceholder:
      "Enter names separated by commas (e.g. Anna, Ben, Clara)",
...
export default function Home() {
  ...
  const importBulkNames = () => {
    ...
    // Parse names separated by commas or newlines, strip quotes and trim whitespace
    const names = bulkInput
      .split(/[,\n]+/)
      .map((name) => name.trim())
      .map((name) => name.replace(/^["']+|["']+$/g, "")) // Remove surrounding quotes
      .map((name) => name.trim())
      .filter((name) => name !== "");

    if (names.length === 0) return;

    // Get existing non-empty participants
    const existingNames = participants.filter((p) => p.trim() !== "");

    // Combine existing and new names
    const allNames = [...existingNames, ...names];

    // Ensure we have at least 3 slots
    while (allNames.length < 3) {
      allNames.push("");
    }
    ...
    <div className="min-h-screen bg-[#FDFB9C] p-4 md:p-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-12 mt-8">
          ...
          <p className="text-2xl font-bold text-black">{t.subtitle}</p>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent className="border-4 border-black">
                  <SelectItem value="de" className="text-lg font-bold">
                    <span
                      style={{
                        fontFamily:
                  ...
                  <SelectItem value="en" className="text-lg font-bold">
                    <span
                      style={{
                        fontFamily:
                          '"Noto Color Emoji", "Apple Color Emoji", "Segoe UI Emoji", sans-serif',
            ...
            <div className="space-y-2">
                ...
                {participants.map((participant, index) => (
                  <div key={index} className="flex gap-2">
                    <Input
                      ...
                      placeholder={`${t.participantPlaceholder} ${index + 1}`}
              ...
              <Button
                ...
                variant="outline"
                className="w-full border-4 border-black bg-white hover:bg-gray-100 h-10 text-base font-bold shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] transition-all"
              >
              ...
              {showBulkImport && (
                <div className="flex gap-2">
                  <Input
                    ...
                    className="border-4 border-black h-12 text-lg font-bold shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] focus-visible:ring-0 focus-visible:ring-offset-0 flex-1"
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        importBulkNames();
                  ...
                  />
                  <Button
                    onClick={importBulkNames}
                    variant="outline"
                    disabled={!bulkInput.trim()}
                    className="border-4 border-black bg-[#FFD93D] hover:bg-[#FFC700] h-12 px-4 text-lg font-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] transition-all disabled:opacity-50"
                  >
                    {t.importNames}
                  </Button>
                </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={currency}
                  onValueChange={(value) =>
                    setCurrency(value as "EUR" | "USD" | "GBP" | "CHF")
                  }
                >
                  <SelectTrigger
                  ...
                  <SelectContent className="border-4 border-black">
                    <SelectItem value="EUR" className="text-lg font-bold">
                      EUR €
                    </SelectItem>
                    <SelectItem value="USD" className="text-lg font-bold">
                      USD $
                    </SelectItem>
                    <SelectItem value="GBP" className="text-lg font-bold">
                      GBP £
                    </SelectItem>
                    <SelectItem value="CHF" className="text-lg font-bold">
                      CHF
                    </SelectItem>
                  </SelectContent>
                </Select>
