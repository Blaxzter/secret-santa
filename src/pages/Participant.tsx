...
export default function Participant() {
  ...
  const [step, setStep] = useState<"initial" | "reveal" | "wishes">("initial");
  ...
  // Skip reveal step if user has already viewed (but not if they're replaying)
  React.useEffect(() => {
    if (assignment?.has_viewed && step === "initial" && !isReplaying) {
      setStep("wishes");
    }
  }, [assignment?.has_viewed, step, isReplaying]);
      ...
      <div className="min-h-screen bg-[#A8E6CF] flex items-center justify-center">
        <Card className="bg-white border-4 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] p-12">
          <div className="text-center">
            ...
            <p className="text-2xl font-black text-black">{t.loading}</p>
          </div>
        </Card>
      </div>
    );
  ...
  if (!assignment || !room) {
    return (
      <div className="min-h-screen bg-[#FF6B9D] flex items-center justify-center p-4">
        <Card className="bg-white border-4 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] p-8">
          <div className="text-6xl mb-4 text-center">😕</div>
          <p className="text-2xl font-black text-center">{t.invalidLink}</p>
        </Card>
      </div>
    );
  }
  ...
  if (step === "wishes") {
    return (
      <WishesStep
        ...
        drawnParticipantWishes={assignment.drawn_participant_wishes || []}
        room={room}
