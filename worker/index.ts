...
async function createRoom(env: Env, request: Request): Promise<Response> {
  ...
  if (!data.owner_id) {
    return jsonResponse({ error: "owner_id is required" }, 400);
  }
  ...
  await env.secret_santa_db
    .prepare(
      `INSERT INTO rooms (id, room_name, participant_names, price_limit, currency, language, is_drawn, owner_id, created_date)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .bind(
      id,
...
async function updateRoom(
  ...
  // Verify ownership
  const { results: existingRoom } = await env.secret_santa_db
  ...
  if ((existingRoom[0] as any).owner_id !== data.owner_id) {
...
async function deleteRoom(
  ...
  // First check if room exists and verify ownership
  const { results } = await env.secret_santa_db
    .prepare("SELECT owner_id FROM rooms WHERE id = ?")
    .bind(id)
    .all();
  ...
  if ((results[0] as any).owner_id !== ownerId) {
    return jsonResponse({ error: "Unauthorized" }, 403);
  }
    ...
    try {
      ...
      if (url.pathname.match(/^\/api\/rooms\/[^/]+$/)) {
        ...
        if (request.method === "DELETE") {
          return await deleteRoom(env, id, request);
        }
      }
      ...
      if (url.pathname.match(/^\/api\/rooms\/[^/]+\/reshuffle$/)) {
        ...
        if (request.method === "POST") {
          return await reshuffleRoom(env, id, request);
        }
      }
